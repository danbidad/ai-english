...
import * as ytcap from "./transcript/index.js"
...
youtubeRouter.get('/subtitles/:videoId', endpoint_GetSubTitles);
youtubeRouter.get('/subtitles/:videoId/:lang', endpoint_GetSubTitles)
...
async function endpoint_GetSubTitles(req, res) {
    try {
        const { videoId, lang } = req.params;
        const details: VideoDetails = await getVideoDetails({
            videoID: videoId,
            lang: lang ?? 'en'
        });
