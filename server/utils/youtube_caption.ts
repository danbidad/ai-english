...
async function fetchInnerTube(endpoint: string, data: any): Promise<any> {
  ...
  const response = await cachedFetch(url, {
    ...
    // @ts-ignore - TypeScript 표준 fetch 타입에는 'agent' 속성이 없으므로, 타입 에러를 무시합니다.
    agent: httpsAgent,
  }, '.ytcap_cache', false);
...
async function getSubtitlesFromCaptions(
  ...
  const proxy = proxyRotator.getNext(); // 프록시를 사용하여 자막을 가져옵니다.
  const httpsAgent = proxy ? new HttpsProxyAgent(proxy) : undefined;
  if (httpsAgent) {
    debug(`Using proxy for captions: ${proxy}`);
  }

  const response = await cachedFetch(captionUrl, {
    headers: {
      'User-Agent':
...
export const getSubtitles = async ({
  ...
  try {
    ...
    // 참여 패널(transcript API) 방식을 먼저 시도합니다.
    if (nextData) {
      try {
    ...
    // 실패 시 기존 자막(captions) 방식으로 대체합니다.
